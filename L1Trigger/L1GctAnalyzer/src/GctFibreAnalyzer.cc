...
GctFibreAnalyzer::GctFibreAnalyzer(const edm::ParameterSet &iConfig):
    ...
    m_doElectronIDs(iConfig.getUntrackedParameter<bool>("doElectronIDs")),
    m_doJetIDs(iConfig.getUntrackedParameter<bool>("doJetIDs")),
...
void GctFibreAnalyzer::CheckLogicalID(const L1GctFibreWord fibre)
{
    ...
    int fibre_id_expected, fibre_id_read;
    ...
    if (((fibre.block() >> 8) & 0x7) == 0 || ((fibre.block() >> 8) & 0x7) == 4)   //i.e. electron leaf cards
    {
        ...
        fibre_id_expected = (2 + fibre.index() % 3);

        fibre_id_read = (fibre.data() & 0xFF);


        edm::LogInfo("Electron leaf card") << " phi_reg:    " << rct_phi_region << "\n"
                                           << " sc_id:      " << source_card_id_expected<< "\n"
                                           << " sc_id_read: " << source_card_id_read << "   [hex: 0x" << std::hex << source_card_id_read << std::dec << "]\n"
                                           << " fb_id:      " << fibre_id_expected<< "\n"
                                           << " fb_id_read: " << fibre_id_read << "   [hex: 0x" << std::hex << fibre_id_read << std::dec << "]"; 

        if (source_card_id_expected != source_card_id_read )
        {
            edm::LogError("GCT fibre data error") << "Electron Source Card IDs do not match "
        ...
        {
            edm::LogError("GCT fibre data error") << "Electron Fibres do not match "
    ...
    else  //i.e. jet leaf cards
    {
        ...
        if (jf_type == -1)
        {
            ...
            if (fibre.index() <= 5 )  //the compiler warning is because fibre.index() is unsigned int and hence is always >=0
            {
                ...
                fibre_id_expected = ref_eta0_link[fibre.index()];

                fibre_id_read = (fibre.data() & 0xFF);

                if ( ref_eta0_type[ fibre.index() ] == 2  && fibre_id_expected > 2) {
                    int old_fb_id = fibre_id_expected;
                    fibre_id_expected -= 2;
                    edm::LogInfo("Duplicated fiber detected") << "old fb id = " << old_fb_id << " -> new fb id = " << fibre_id_expected;
                }

                edm::LogInfo("Jet leaf card") << " phi_reg:    " << rct_phi_region << "\n"
                                           << " sc_type:    " << ref_eta0_type[ fibre.index() ] << "\n"
                                           << " sc_id:      " << source_card_id_expected<< "\n"
                                           << " sc_id_read: " << source_card_id_read << "   [hex: 0x" << std::hex << source_card_id_read << std::dec << "]\n"
                                           << " fb_id:      " << fibre_id_expected<< "\n"
                                           << " fb_id_read: " << fibre_id_read << "   [hex: 0x" << std::hex << fibre_id_read << std::dec << "]"; 
                ...
                if (source_card_id_expected != source_card_id_read )
                {
                    edm::LogError("GCT fibre data error") << "ETA0 Source Card IDs do not match "
                                                         << "Expected ID = " << source_card_id_expected
                                                         << " ID read from data = " << source_card_id_read
                                                         << " " << fibre; //screwed up
                }

                if ( fibre_id_read != fibre_id_expected)
                {
                    edm::LogError("GCT fibre data error") << "ETA0 Fibres do not match "
                                                         << "Expected Fibre = " << fibre_id_expected
                                                         << " Fibre read from data = " << fibre_id_read
                                                         << " " << fibre; //screwed up
                }
            }
            else edm::LogError("GCT fibre data error") << "ETA0 Fibre index out of bounds " << fibre;
        ...
        if (jf_type >= 0)
        {
            if (fibre.index() <= 7 )
            {
                ...
                fibre_id_expected = ref_jf_link[fibre.index()];

                fibre_id_read = (fibre.data() & 0xFF);

                if ( ref_jf_type[ fibre.index() ] == 2  && fibre_id_expected > 2) {
                    int old_fb_id = fibre_id_expected;
                    fibre_id_expected -= 2;
                    edm::LogInfo("Duplicated fiber detected") << "old fb id = " << old_fb_id << " -> new fb id = " << fibre_id_expected;
                }
                ...
                edm::LogInfo("Jet leaf card") << " phi_reg:    " << rct_phi_region << "\n"
                                           << " sc_type:    " << ref_jf_type[ fibre.index() ] << "\n"
                                           << " sc_id:      " << source_card_id_expected<< "\n"
                                           << " sc_id_read: " << source_card_id_read << "   [hex: 0x" << std::hex << source_card_id_read << std::dec << "]\n"
                                           << " fb_id:      " << fibre_id_expected<< "\n"
                                           << " fb_id_read: " << fibre_id_read << "   [hex: 0x" << std::hex << fibre_id_read << std::dec << "]"; 
                ...
                if (source_card_id_expected != source_card_id_read )
                {
                    edm::LogError("GCT fibre data error") << "Source Card IDs do not match "
                                                         << "Expected ID = " << source_card_id_expected
                                                         << " ID read from data = " << source_card_id_read
                                                         << " " << fibre; //screwed up
                }
                ...
                if ( fibre_id_read != fibre_id_expected )
                {
                    edm::LogError("GCT fibre data error") << "Fibres do not match "
                                                         << "Expected Fibre = " << fibre_id_expected
                                                         << " Fibre read from data = " << fibre_id_read
                                                         << " " << fibre; //screwed up
                }
            }
            else edm::LogError("GCT fibre data error") << "Fibre index out of bounds " << fibre;
        }
